...
class TRISO:
    ...
    def volume(self):
        ...
        self.dv_buffer = 4./3.*math.pi * \
            (math.pow(self.r_buffer, 3.) - math.pow(self.r_kernel, 3.))
        self.dv_iPyC = 4./3.*math.pi * \
            (math.pow(self.r_iPyC, 3.) - math.pow(self.r_buffer, 3.))
        self.dv_SiC = 4./3.*math.pi * \
            (math.pow(self.r_SiC, 3.) - math.pow(self.r_iPyC, 3.))
        self.dv_oPyC = 4./3.*math.pi * \
            (math.pow(self.r_oPyC, 3.) - math.pow(self.r_SiC, 3.))
        self.dv_matrix = 4./3.*math.pi * \
            (math.pow(self.r_matrix, 3.) - math.pow(self.r_oPyC, 3.))
        self.V = self.dv_kernel + self.dv_buffer + self.dv_iPyC + \
            self.dv_SiC + self.dv_oPyC + self.dv_matrix
    ...
    def homogenize(self):
        ...
        self.mat_homogenized = self.mat_kernel*self.dv_kernel + self.mat_buffer*self.dv_buffer + self.mat_iPyC * \
            self.dv_iPyC + self.mat_SiC*self.dv_SiC + self.mat_oPyC*self.dv_oPyC + self.mat_matrix*self.dv_matrix
    ...
    def CHM(self):
        self.homogenize()
        return (
            self.mat_homogenized.comp['60120'] /
            self.mat_homogenized.heavy_moles())
    ...
    def conductivities(self):
        ...
        k = (
            self.k_kernel * self.dv_kernel + self.k_buffer * self.dv_buffer +
            self.k_iPyC * self.dv_iPyC + self.k_SiC * self.dv_SiC + self.k_oPyC *
            self.dv_oPyC + self.k_matrix * self.dv_matrix) / self.V
        ...
        while math.fabs(k_errs['mid']) > self.max_k_err:
            ks['mid'] = (ks['min']-ks['max']) / \
                (k_errs['min']-k_errs['max'])*(-k_errs['max'])+ks['max']
    ...
    def HT(self, Q):
        '''
        This method returns the average temperature perturbation in the fuel
        kernel, ave_T and the maximum temperature perturbation in the fuel
        kernel, max_T
        '''
        ...
        at_buffer = Q*(3/(8*math.pi*k)*(math.pow(r2, 2.) - math.pow(r1, 2.))/
                       (math.pow(r2, 3.) - math.pow(r1, 3.))-1/(4.*math.pi*k*r2))
        ...
        at_iPyC = Q*(3/(8*math.pi*k)*(math.pow(r2, 2.) - math.pow(r1, 2.))/
                     (math.pow(r2, 3.) - math.pow(r1, 3.))-1/(4.*math.pi*k*r2))
        ...
        at_SiC = Q*(3/(8*math.pi*k)*(math.pow(r2,
                                              2.) - math.pow(r1,
                                                             2.))/(math.pow(r2,
                                                                            3.) - math.pow(r1,
                                                                                           3.))-1/(4.*math.pi*k*r2))
        ...
        at_oPyC = Q*(3/(8*math.pi*k)*(math.pow(r2,
                                               2.) - math.pow(r1,
                                                              2.))/(math.pow(r2,
                                                                             3.) - math.pow(r1,
                                                                                            3.))-1/(4.*math.pi*k*r2))
        ...
        at_matrix = Q*(3/(8*math.pi*k)*(math.pow(r2,
                                                 2.) - math.pow(r1,
                                                                2.))/(math.pow(r2,
                                                                               3.) - math.pow(r1,
                                                                                              3.))-1/(4.*math.pi*k*r2))
        ...
        T_kernel = dt_matrix + dt_oPyC + dt_SiC + \
            dt_iPyC + dt_buffer + at_kernel

        offset = -(T_matrix * self.dv_matrix + T_oPyC * self.dv_oPyC + T_SiC *
                   self.dv_SiC + T_iPyC * self.dv_iPyC + T_buffer * self.
                   dv_buffer + T_kernel * self.dv_kernel) / self.V
    ...
    def matrix(self):
        ...
        self.homogenize()
        self.mat_hmatrix = (
            self.mat_homogenized*self.V + self.mat_kernel*-self.dv_kernel)*(1./(self.V - self.dv_kernel))
